...
Description: Preloads images using Javascript. Use it to enhance your blog's visitors experience by preloading images for hover effects, slideshows etc.
Author: Adrian &#350;ilimon
Version: 1.2
Author URI: http://adrian.silimon.eu/
License: GPL2

	Copyright 2011  Adrian Silimon  (email: adrian.silimon@yahoo.com)
					...
					<table class="form-table">
						...
						<tr valign="top">
							...
							<td>
								...
								' . __("* Example:", WP_IMG_PRELOADER_TEXTDOMAIN) . ' [preload=http://domain.tld/bigimage.png] to preload one image
...
function wpimgpreloader_parse_stack_from_content($content=""){
	...
	if (count($matches)) foreach ($matches as $m){
		...
		//check if there is a single url or a list of urls separated by commas
		$urls = array();
		
		if (strrpos($url, ',')  === FALSE){
			$urls[] = wpimgpreloader_parse_url($url);
		}
		else{
			$list = @explode(",", $url);
			
			if(is_array($list) and count($list)) foreach ($list as $url) $urls[] = wpimgpreloader_parse_url($url);
		}
			
		$WPIMGP_Queue = array_merge($WPIMGP_Queue, $urls);
